...
CUATRO_inst = CUATRO(sampling='g', explore=None, method='local', N_min_samples=6, beta_red=beta, tolerance=tol, init_radius=init_radius, dim_red='PLS')
CUATRO_embed = CUATRO(sampling='g', explore=None, method='local', N_min_samples=6, beta_red=beta, tolerance=tol, init_radius=init_radius, dim_red='embed')
CUATRO_PLS_expl = CUATRO(sampling='base', explore=None, method='local', N_min_samples=6, beta_red=beta, tolerance=tol, init_radius=init_radius, dim_red='explore')
CUATRO_PLS_bandit = CUATRO(sampling='base', explore=None, method='local', N_min_samples=6, beta_red=beta, tolerance=tol, init_radius=init_radius, dim_red='bandit')
...
class TestConvergence_PLS_quadr(unittest.TestCase):   
    def runTest(self):
        ...
        self.assertTrue((abs(res['f_best_so_far'][-1] - optimum)**2 < conv_rad), f"CUATRO_PLS did not converge to {conv_rad} accuracy for a budget of {N}; best_f: {res['f_best_so_far'][-1]}, optimum: {optimum}")

class TestConvergence_PLS_expl_quadr(unittest.TestCase):   
    def runTest(self):
        """
        check whether convergence is achieved for the quadratic test function in high dimensions
        """  
        res = CUATRO_PLS_expl.run_optimiser(sim=f_quadr, x0=x0, bounds=bounds, max_f_eval=N, rnd=0, n_pls=N_pls, n_t=n_t)
        print(f"Objective: {res['f_best_so_far'][-1]}")
        self.assertTrue((abs(res['f_best_so_far'][-1] - optimum)**2 < conv_rad), f"CUATRO_PLS_explore did not converge to {conv_rad} accuracy for a budget of {N}; best_f: {res['f_best_so_far'][-1]}, optimum: {optimum}")

class TestConvergence_PLS_bandit(unittest.TestCase):   
    def runTest(self):
        """
        check whether convergence is achieved for the quadratic test function in high dimensions
        """  
        res = CUATRO_PLS_bandit.run_optimiser(sim=f_quadr, x0=x0, bounds=bounds, max_f_eval=N, rnd=0, n_pls=N_pls, n_t=n_t)
        print(f"Objective: {res['f_best_so_far'][-1]}")
        self.assertTrue((abs(res['f_best_so_far'][-1] - optimum)**2 < conv_rad), f"CUATRO_PLS_explore did not converge to {conv_rad} accuracy for a budget of {N}; best_f: {res['f_best_so_far'][-1]}, optimum: {optimum}")


class TestConvergence_PLS_embed(unittest.TestCase):   
    def runTest(self):
        """
        check whether convergence is achieved for the quadratic test function in high dimensions
        """  
        res = CUATRO_embed.run_optimiser(sim=f_quadr, x0=x0, bounds=bounds, max_f_eval=N, rnd=0, n_pls=N_pls)
        print(f"Objective: {res['f_best_so_far'][-1]}")
        self.assertTrue((abs(res['f_best_so_far'][-1] - optimum)**2 < conv_rad), f"CUATRO_embed did not converge to {conv_rad} accuracy for a budget of {N}; best_f: {res['f_best_so_far'][-1]}, optimum: {optimum}")
