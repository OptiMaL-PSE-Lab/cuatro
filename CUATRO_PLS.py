...
def samples_in_trust(center, radius, \
    ...
    ind = np.where(np.linalg.norm(X - center, axis = 1,\
...
def CUATRO_PLS(sim, x0, init_radius, constraints = [], bounds = None, \
           ...
           constr_handling = 'Discrimination', pls_n = 2):
    '''
    INPUTS
    ------------------------------------
    ...
    best_x_pls = X_samples_list_pls.copy()
    ...
    # t = time.process_time()
    pls2 = PLSRegression(n_components=pls_n)
    ...
    X_samples_list_pls += X_samples_pls.tolist()
    f_eval_list += y_samples
    g_eval_list += g_eval
    
    center_pls = pls2.transform(np.array(center).reshape(1, -1)).squeeze()
    old_trust = center_pls
    X_samples_list_pls += old_trust.reshape(1,-1).tolist()
    old_f = best_f[0]

    # t = time.process_time()
    P, q, r = quadratic_fitting(X_samples_pls, np.array(y_samples))
    feas_X = X_samples_pls.copy()[feas == 1]
    infeas_X = X_samples_pls.copy()[feas != 1]

    if not ((P is None) or (q is None) or (r is None)):
        center_ = minimise(X_samples_pls, feas_X, infeas_X, np.array(g_eval), P, q, \
                           r, bounds, center_pls, radius, constr_handling)
    else:
    ...
    f_eval, g_eval, new_feas = sample_simulation(center, sim) # in lower_dim domain
    ...
    # print(new_feas)
    X_samples_list_pls += [center]
    X_samples_list += pls2.inverse_transform(np.array(center).reshape(1,-1)).tolist()
    f_eval_list += [new_f]
    g_eval_list += g_eval
    ...
    best_x_pls, best_f, best_g = update_best_lists(X_samples_list_pls, \
                                               f_eval_list, g_eval_list,  \
                                               best_x_pls, best_f, best_g)
    ...
    while (len(f_eval_list) < max_f_eval - 1) and (N <= max_iter) and (radius > tolerance):
        ...
        X_in_trust, y_in_trust, g_in_trust, feas_in_trust = samples_in_trust(center, radius, \ 
                                                                X_samples_list_pls, f_eval_list, g_eval_list)
        ...
        center = pls2.inverse_transform(np.array(center).reshape(1,-1)).squeeze()
        # center_ = list(x0) 
        ...
        X_samples_pls = np.array(X_in_trust.tolist() + X_samples_pls.tolist())
        ...
        try:
            P, q, r = quadratic_fitting(X_samples_pls, y_samples)
        except:
        ...
        if not ((P is None) or (q is None) or (r is None)):
            ...
            center_ = minimise(X_samples_pls, feas_X, infeas_X, g_samples, P, q, r, bounds, \
                           center_pls, radius, constr_handling)
            
            center = [float(c) for c in center_]
            
            f_eval, g_eval, new_feas = sample_simulation(center, sim) # low
            new_f = f_eval[0]
            
            # center = pls2.transform(np.array(center).reshape(1,-1)).squeeze()
            X_samples_list_pls += [center]
            X_samples_list += pls2.inverse_transform(np.array(center).reshape(1,-1)).tolist()
            f_eval_list += [new_f]
            g_eval_list += g_eval
            X = np.array(center_pls).reshape(-1,1)
            new_pred_f = X.T @ P @ X + q.T @ X + r
    ...
    # t_center = time.process_time() - t1
    # print("Update new center time: ", t_center)
    # print("Sampling time: ", t_sampling)
    # print("PLS fitting time: ", t_pls_fit)
    # print("Quadratic fitting time is: ", t_quad_fit) 
    # print("Query of one point time is: ", t_sim_1pt)
    # print("Time of each iteration is: ", t_it)
    # print("Time to select a new center: ", t_center_select)
    # print("Time to sample new points in trust region: ", t_sample)
    # print("Time to minimise one surrogate at each iteration: ", t_minimise)
    ...
    output = {'x_best_so_far': best_x, 'f_best_so_far': best_f, \
              ...
              'x_best_so_far_pls': best_x_pls, 'x_store_pls': X_samples_list_pls}
